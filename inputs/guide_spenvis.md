...
**画面キャプチャの注記:** SPENVIS の操作画面を撮影した図には、画面上の**コピーライト・クレジット等の表記**が含まれます。権利は SPENVIS の開発・保守主体（ベルギー王立宇宙気象研究所 BIRA-IASB を中心とするコンソーシアム・ESA 等）に帰属します。再利用・転載時は [SPENVIS](https://www.spenvis.oma.be) の利用条件を確認し、画面上の表記を改変しないでください。
...
5. **レポート・テキスト出力**  
   画面左の **Report** から PDF を取得できる（セル種によりファイル名が変わる）。シリコン計算の例は `Solar_cell_damage_equivalent_fluences.txt`、三接合 AZURE 3G30 の例は `MJ_Solar_cell_damage_equivalent_fluences.pdf` およびテキスト `MJ_Solar_cell_damage_equivalent_fluences` に相当する出力である。
...
**シリコン（Si）** — `Solar_cell_damage_equivalent_fluences.txt`
...
**三接合（AZURE 3G30）** — `MJ_Solar_cell_damage_equivalent_fluences`／`MJ_Solar_cell_damage_equivalent_fluences.pdf`
