...
**SPENVIS 画面キャプチャについて:** 操作手順用のスクリーンショット（`inputs/images/` のうち SPENVIS Web 画面の撮影にあたる図）には、画面上に表示されている**コピーライト・クレジット・利用に関する表記**が含まれます。権利は **SPENVIS の開発・保守主体（ベルギー王立宇宙気象研究所 BIRA-IASB を中心とするコンソーシアム・ESA 等、サービスが示す権利者）** に帰属します。第三者への提供や再掲載を行う場合は、[SPENVIS のサイト](https://www.spenvis.oma.be)で示される利用条件・著作権ポリシーを確認し、画面上の表記を改変・削除しないよう留意してください。

**本文の体裁:** *Table n …*（表題）は各表の**直上**、*Fig. n …*（図題）は各画像の**直下**に付す。
...
| [inputs/Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt](./inputs/Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt) | シリコンセル・EQFULX（ISS 軌道・0.5 年相当、§9.3）。 |
...
| [inputs/MJ_Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt](./inputs/MJ_Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt) | 三接合（AZURE 3G30）・EQFULX（ISS 軌道・0.5 年相当、§9.3）。 |
...
- [６．適用例 1：GEO 軌道（10 年）](#readme-sec-6)
- [７．適用例 2：ISS 軌道（180 日）](#readme-sec-7)
- [８．GEO と ISS の比較](#readme-sec-8)
...
[SPENVIS](https://www.spenvis.oma.be)（**Space Environment, Effects, and Education System**。ESA では **SPace ENVironment Information System** と説明される）は、銀河宇宙線・太陽高エネルギー粒子・自然放射線帯・プラズマ・気体・流星体／デブリなど、**宇宙環境とその影響**に関する多数のモデルへアクセスする Web インタフェースである。本リポジトリでは放射線環境、帯電、大気、磁場などと記したが、公式が列挙する範囲に含まれる。利用は**無料**だが**ユーザー登録が必須**である。開発は [BIRA-IASB](https://www.aeronomie.be/) を中心とするコンソーシアムが ESA の宇宙環境・影響部門（GSTP 等を通じた支援）のもとで行い、保守も同研究所が担う。ブラウザは **JavaScript 対応**が必要とされ、一部出力では **VRML/X3D** プラグインが示される場合がある（いずれも[公式サイト](https://www.spenvis.oma.be)の記載に従う）。バージョン番号・リリース日はサイトの *Current version* 表記が最新である。
...
*Fig. 1: Model Packages ページ*

### Step 2: ミッション定義

Mission duration を設定する。**Representative trajectory duration を「1 days」に設定**することが重要。特に ISS などの LEO 軌道では、この値が小さいと SAA 通過がサンプリングされず、捕捉粒子フラックスがゼロとなる。

![Mission definition](./inputs/images/fig02_mission_def.png)

*Fig. 2: Mission definition 画面*
...
*Fig. 3: 軌道タイプ選択（GEO の例）*

### Step 4: 軌道生成・確認

設定確認後、Run をクリックし、Mission overview でパラメータを確認する。

![Mission overview](./inputs/images/fig04_mission_overview.png)

*Fig. 4: Mission overview（GEO の例）*
...
*Fig. 5: Tables and plots ページ（右上 Output から開く）*
...
| Electron model | AE-8 MIN / MAX | Dd 計算に必要 |
...
| Energies | default SPENVIS energies | AP-8 と同じグリッドで比較しやすい |
...
*Fig. 10: GEO 10 年間 Combined Proton Fluence（AP-9 + ESP-PSYCHIC）*

---

<a id="readme-sec-7"></a>

## ７．適用例 2：ISS 軌道（180 日）

### 7.1 計算条件

*Table: ISS 180 日適用例の計算条件*
...
**重要:** ISS 軌道では Representative trajectory duration を必ず **1 day** に設定する。ISS の軌道周期は約 1.55 時間であり、1 周回や数分では SAA 通過がサンプリングされず、捕捉粒子フラックスがゼロとなる。

### 7.2 結果

ISS 軌道では太陽陽子線フルエンスが **全エネルギーでゼロ** となった。地磁気遮蔽の減衰率が 0.0009（99.9% 遮蔽）であり、太陽陽子線はほぼ到達しない。したがって、**ISS 軌道の Combined proton = Trapped proton のみ**である。捕捉陽子線は 0.1〜300 MeV の広いエネルギー範囲にわたり、SAA 通過による軌道平均フルエンスである。

![ISS combined](./inputs/images/fig11_iss_combined.png)

*Fig. 11: ISS 180 日間 Combined Proton Fluence（= Trapped only）*
...
GEO と ISS の捕捉陽子線フラックスを比較すると、以下の特徴が確認できる。
...
テキスト出力の **Summary of Equivalent Fluences …, Pmax**（深度＝カバーガラス表面からの厚さ、単位 cm⁻²）を、**シリコン単結晶**と**三接合 AZURE 3G30**（SPENVIS のセル種名は *Azur 3G30 (SR-NIEL 21eV)*）について同じ形式で示す。**Table 9・10** は軌道・ミッション期間・放射線モデルを §6 の GEO 10 年例に揃えた結果である。**Table 11・12** は §7 と同一の ISS 軌道・約 0.5 年条件の結果である（本文後段）。GEO では捕捉陽子が極厚ガラスで実質ゼロに近づく一方、太陽陽子・捕捉電子の寄与が残る傾向があるが、総量・成分比はセル種および軌道条件で異なる。
...
ISS 軌道（§7 と同一の軌道・期間条件：**410 km・51.6°・180 日（約 0.5 年）**、代表軌道サンプリング **1 day**）における見積もりとして、捕捉陽子 **AP-8 MAX**、捕捉電子 **AE-8 MAX**、太陽陽子 **ESP-PSYCHIC（90% CL・磁気遮蔽あり）** とした EQFULX の **Summary of Equivalent Fluences …, Pmax** を、[Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt](./inputs/Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt) および [MJ_Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt](./inputs/MJ_Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt) に対応する出力から Table 11・12 に示す。太陽陽子の寄与は §7 と同様に実質ゼロであり、SPENVIS のサマリーでは数値下限（約 10⁻¹² cm⁻²）が出力されるため、表中では **≈ 0** とした。

**シリコン（Si）** — [Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt](./inputs/Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt)

*Table 11: ISS 軌道・約 0.5 年における Si の 1 MeV 電子等価フルエンス（Pmax）のカバーガラス厚さ依存*

| Cover glass (μm) | Total Φ_eq | Trapped protons | Solar protons | Trapped electrons |
|------------------|------------|-----------------|---------------|-------------------|
| 0 | 8.81 × 10¹² | 8.77 × 10¹² | ≈ 0 † | 3.56 × 10¹⁰ |
| 25.4 | 5.98 × 10¹¹ | 5.68 × 10¹¹ | ≈ 0 † | 3.08 × 10¹⁰ |
| 76.2 | 3.15 × 10¹¹ | 2.89 × 10¹¹ | ≈ 0 † | 2.57 × 10¹⁰ |
| 152.4 | 2.24 × 10¹¹ | 2.03 × 10¹¹ | ≈ 0 † | 2.12 × 10¹⁰ |
| 304.8 | 1.67 × 10¹¹ | 1.51 × 10¹¹ | ≈ 0 † | 1.60 × 10¹⁰ |
| 508 | 1.32 × 10¹¹ | 1.20 × 10¹¹ | ≈ 0 † | 1.20 × 10¹⁰ |
| 762 | 1.14 × 10¹¹ | 1.05 × 10¹¹ | ≈ 0 † | 8.97 × 10⁹ |
| 1524 | 9.30 × 10¹⁰ | 8.86 × 10¹⁰ | ≈ 0 † | 4.38 × 10⁹ |

**三接合（AZURE 3G30）** — [MJ_Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt](./inputs/MJ_Solar_cell_damage_equivalent_fluences_in_ISS_05yr.txt)

*Table 12: ISS 軌道・約 0.5 年における AZURE（Azur）3G30 の 1 MeV 電子等価フルエンス（Pmax）のカバーガラス厚さ依存*

| Cover glass (μm) | Total Φ_eq | Trapped protons | Solar protons | Trapped electrons |
|------------------|------------|-----------------|---------------|-------------------|
| 0 | 4.57 × 10¹³ | 4.57 × 10¹³ | ≈ 0 † | 2.10 × 10¹⁰ |
| 25.4 | 2.73 × 10¹¹ | 2.54 × 10¹¹ | ≈ 0 † | 1.90 × 10¹⁰ |
| 76.2 | 1.13 × 10¹¹ | 9.64 × 10¹⁰ | ≈ 0 † | 1.66 × 10¹⁰ |
| 152.4 | 8.09 × 10¹⁰ | 6.65 × 10¹⁰ | ≈ 0 † | 1.43 × 10¹⁰ |
| 304.8 | 6.18 × 10¹⁰ | 5.05 × 10¹⁰ | ≈ 0 † | 1.13 × 10¹⁰ |
| 508 | 5.17 × 10¹⁰ | 4.30 × 10¹⁰ | ≈ 0 † | 8.71 × 10⁹ |
| 762 | 4.56 × 10¹⁰ | 3.90 × 10¹⁰ | ≈ 0 † | 6.57 × 10⁹ |
| 1524 | 3.64 × 10¹⁰ | 3.32 × 10¹⁰ | ≈ 0 † | 3.19 × 10⁹ |

† ISS 軌道では太陽陽子の等価フルエンスは §7 のとおり実質ゼロである。出力上は約 10⁻¹² cm⁻² のプレースホルダが現れるため、本表では ≈ 0 と記載した。
...
**軌道サンプリング:** LEO 軌道では Representative trajectory duration を 1 day に設定すること。短い設定では SAA がサンプルされず、捕捉粒子フラックスがゼロとなる。
...
[1] SPENVIS（Space Environment, Effects, and Education System）— ESA の宇宙環境情報システム, https://www.spenvis.oma.be
